...
/* @test
 * @build TestSocketFactory
 * @run testng/othervm TestSocketFactory
 */
...
public class TestSocketFactory extends RMISocketFactory
    ...
    private transient final List<InterposeSocket> sockets = new ArrayList<InterposeSocket>();

    private transient final List<InterposeServerSocket> serverSockets = new ArrayList<InterposeServerSocket>();
    ...
    public void setMatchReplaceBytes(byte[] matchBytes, byte[] replaceBytes) {
        this.matchBytes = requireNonNull(matchBytes, "matchBytes");
        this.replaceBytes = requireNonNull(replaceBytes, "replaceBytes");
        for (InterposeSocket s : sockets) {
            s.setMatchReplaceBytes(matchBytes, replaceBytes);
        }
        for (InterposeServerSocket s : serverSockets) {
            s.setMatchReplaceBytes(matchBytes, replaceBytes);
        }
    }
    ...
    public List<InterposeSocket> getSockets() {
        List<InterposeSocket> snap = new ArrayList<InterposeSocket>(sockets);
        return snap;
    }
    ...
    public List<InterposeServerSocket> getServerSockets() {
        List<InterposeServerSocket> snap = new ArrayList<InterposeServerSocket>(serverSockets);
        return snap;
    }
    ...
    public static class InterposeServerSocket extends ServerSocket {
        ...
        private volatile byte[] matchBytes;
        private volatile byte[] replaceBytes;
        private final List<InterposeSocket> sockets = new ArrayList<InterposeSocket>();
    ...
    public static void test3(byte[] match, byte[] replace,
        ...
        try {
            ...
            out = new MatchReplaceOutputStream(output, "test3", log, match, replace);
            out.write(input);
