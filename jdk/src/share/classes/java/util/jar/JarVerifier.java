...
class JarVerifier {
    ...
    boolean isTrustedManifestEntry(String name) {
        ...
        CodeSigner[] forMan = (CodeSigner[]) verifiedSigners.get(JarFile.MANIFEST_NAME);
        if (forMan == null) {
            return true;
        }
        // Check sigFileSigners first, because we are mainly dealing with
        // non-file entries which will stay in sigFileSigners forever.
        CodeSigner[] forName = (CodeSigner[]) sigFileSigners.get(name);
        if (forName == null) {
            forName = (CodeSigner[]) verifiedSigners.get(name);
        }
