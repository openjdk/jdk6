    ...
    extends InputStream implements ObjectInput, ObjectStreamConstants
{
    ...
    private static class Logging {
        ...
        static {
            Logger filterLog = Logger.getLogger("java.io.serialization");
            infoLogger = (filterLog != null &&
                filterLog.isLoggable(Level.INFO)) ? filterLog : null;
            traceLogger = (filterLog != null &&
                filterLog.isLoggable(Level.FINER)) ? filterLog : null;
        }
    }
        ...
        if (serialFilter != null) {
            ...
            if (status == null  ||
                ...
                if (Logging.infoLogger != null) {
                    Logging.infoLogger.log(Level.INFO,
                            "ObjectInputFilter {0}: {1}, array length: {2}, nRefs: {3}, depth: {4}, bytes: {5}, ex: {6}",
					   new Object[] {status, clazz, arrayLength, totalObjectRefs, depth, bin.getBytesRead(),
							 objectsToString(ex, "n/a")});
                }
                ...
                if (Logging.traceLogger != null) {
                    Logging.traceLogger.log(Level.FINER,
                            "ObjectInputFilter {0}: {1}, array length: {2}, nRefs: {3}, depth: {4}, bytes: {5}, ex: {6}",
					    new Object[] {status, clazz, arrayLength, totalObjectRefs, depth, bin.getBytesRead(),
							  objectsToString(ex, "n/a")});
                }
