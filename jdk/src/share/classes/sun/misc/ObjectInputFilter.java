...
public interface ObjectInputFilter {
    ...
    final class Config {
         ...
         * Debug: Logger
         */
        private final static Logger configLog;

        /**
         * Logger for debugging.
         */
        static void filterLog(Level level, String msg, Object... args) {
            if (configLog != null) {
                if (Level.INFO.equals(level)) {
                    configLog.log(Level.INFO, msg, args);
                } else if (Level.WARNING.equals(level)) {
                    configLog.log(Level.WARNING, msg, args);
                } else {
                    configLog.log(Level.SEVERE, msg, args);
                }
            }
        }
            ...
            configuredFilter =
                AccessController.doPrivileged(new PrivilegedAction<ObjectInputFilter> () {
                    public ObjectInputFilter run() {
                        ...
                        if (props != null) {
                            Logger log = Logger.getLogger("java.io.serialization");
                            log.log(Level.INFO, "Creating serialization filter from {0}", props);
                            try {
                                return createFilter(props);
                            } catch (RuntimeException re) {
                                log.log(Level.WARNING, "Error configuring filter: {0}", re);
                            }
                        }
                        return null;
                    }
        ...
        public static ObjectInputFilter getObjectInputFilter(ObjectInputStream inputStream) {
            objectsRequireNonNull(inputStream, "inputStream");
            return sun.misc.SharedSecrets.getJavaOISAccess().getObjectInputFilter(inputStream);
        }

	private static <T> T objectsRequireNonNull(T obj, String message) {
	    if (obj == null)
        ...
        public static void setSerialFilter(ObjectInputFilter filter) {
            objectsRequireNonNull(filter, "filter");
	...
	static interface GlobalFilter {
	    ...
	    private final List<GlobalFilter> filters;
			...
			for (GlobalFilter filter : filters) {
			    Status status = filter.apply(cl);
			    if (status == Status.REJECTED || status == Status.ALLOWED) {
				return status;
			    }
			}
			return Status.UNDECIDED;
