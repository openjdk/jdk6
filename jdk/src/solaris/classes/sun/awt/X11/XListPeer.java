...
class XListPeer extends XComponentPeer implements ListPeer, XScrollbarClient {
    ...
    class ListPainter {
        ...
        private void paintItems(Graphics g, int firstItem, int lastItem, int options) {
            if (log.isLoggable(Level.FINER)) {
		log.finer("Painting items from " + firstItem + " to " + lastItem +
			  ", focused " + focusIndex + ", first " + getFirstVisibleItem() +
			  ", last " + getLastVisibleItem());
	    }
        ...
        public void copyArea(int x, int y, int width, int height, int dx, int dy) {
            if (log.isLoggable(Level.FINER)) {
		log.finer("Copying area " + x + ", " + y + " " + width +
			  "x" + height + ", (" + dx + "," + dy + ")");
	    }
            VolatileImage localBuffer = null;
            do {
